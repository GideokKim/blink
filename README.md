...
- **⚡ 즉시 검색** — 트레이에 조용히 상주하다가 단축키 한 번에 스포트라이트 창이
  떠오릅니다. 검색은 백그라운드에서 처리돼 입력 중에도 창이 멈추지 않고, 결과에는
  검색어가 매치된 문장 미리보기가 함께 표시돼요.
...
- **🧹 잡음 없는 결과** — 임시 파일, Office 잠금 파일(`~$…`), OS 메타데이터는 자동
  ...
  있습니다.
- **⬆️ 자동 업데이트** — 새 버전이 나오면 트레이 알림으로 알려주고, 한 번의
  클릭으로 내려받아 설치까지 끝납니다.
...
- 드라이브 루트(`L:\`) 전체를 자식 폴더 단위로 나눠 인덱싱하는 `drive_split` 지원.
  대형 폴더는 크기에 맞춰 적응형 청크로 자동 분할해 누락 없이 안정적으로 색인합니다.
...
**인덱서 워커**는 크로스플랫폼에서 단위 테스트되며(362개), **WPF 앱**(`Blink.App`)은
...
Blink가 시간을 아껴줬다면 커피 한 잔으로 응원해주세요. 앱 안 설정·트레이·"새로워진 점"
화면에서는 카카오페이·토스 QR로도 후원할 수 있습니다. ☕
